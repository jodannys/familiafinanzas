                ...
                <div key={b.id} className="rounded-xl overflow-hidden"
                  ...
                  {bloqueItems.length > 0 && (
                    <div className="px-3 pb-3 space-y-1">
                      {bloqueItems.map(item => (
                        <div key={item.id} className="flex justify-between text-xs px-2 py-1 rounded-lg"
                          style={{ background:`${b.color}08` }}>
                          <span style={{ color:'var(--text-secondary)' }}>{item.nombre}</span>
                          <span className="font-semibold" style={{ color:b.color }}>{formatCurrency(item.monto)}</span>
                        </div>
                      ))}
                      {totalItems > monto && (
                        <p className="text-xs text-center mt-1" style={{ color:'#C0605A' }}>
                          ⚠ Detalle ({formatCurrency(totalItems)}) supera el presupuesto ({formatCurrency(monto)})
                        </p>
                      )}
                    </div>
                  )}
