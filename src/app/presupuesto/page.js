                      ...
                      <div className="pt-3 border-t border-dashed border-stone-200 space-y-2">
                        ...
                        {metasNoAgregadas.map(meta => {
                          ...
                          return (
                            <button key={meta.id}
                              onClick={() => {
                                setAddingReal({ nombre: `🎯 ${meta.nombre}`, montoSugerido: Math.min(restante, monto * (sub.metas / 100)), bloque: 'futuro' })
                        ...
                        {invNoAgregadas.map(inv => (
                          <button key={inv.id}
                            onClick={() => {
                              setAddingReal({ nombre: `📈 ${inv.nombre}`, montoSugerido: inv.aporte || 0, bloque: 'futuro' })
