...
export default function PresupuestoPage() {
  ...
  async function handleAddItem(bloqueId) {
    ...
    const { data, error } = await supabase
      ...
      .select()
    if (!error) { setItems(prev => [...prev, data[0]]); setFormItem({ nombre:'', monto:'' }); setAddingTo(null) }
  }
        ...
        {lista.map((bloque) => {
          ...
          return (
            <Card key={bloque.id} className="animate-enter">
              ...
              {/* Gastado / Disponible — siempre visible si hay ingresos */}
              {ingresoNum > 0 && (
              ...
              {/* Sub-distribución solo para Futuro */}
              {bloque.id === 'futuro' && (
