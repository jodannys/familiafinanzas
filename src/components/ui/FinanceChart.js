...
import { useFormatCurrency } from '@/lib/useFormatCurrency' // FIX 2
...
function leerColores() {
  ...
  return {
    ...
    bgCard: v('--bg-card'),        // FIX 1: usar --bg-card en vez de --bg-dark-card
    ...
    <div style={{
      ...
      {payload.map((p, i) => (
        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
          ...
          {/* FIX 2: usar useFormatCurrency consistente con el resto de la app */}
...
const [colores, setColores] = useState({})
  ...
  // FIX 3: estado vacío
  if (!data || data.length === 0 || data.every(d => d.ingresos === 0 && d.gastos === 0)) {
    return (
      <div
        className="h-[380px] w-full flex items-center justify-center rounded-[40px]"
        style={{
      ...
      >
        <p style={{
          ...
          letterSpacing: '0.15em', opacity: 0.3,
    ...
    >
      ...
      <div style={{ padding: '28px 28px 0 28px' }}>
        <p style={{
          ...
          letterSpacing: '0.25em', color: colores.tickColor, opacity: 0.7, marginBottom: 4,
        }}>
          Progreso Financiero
        </p>
        <div style={{ display: 'flex', gap: 16, marginTop: 8 }}>
          {[
            ...
            <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <span style={{ fontSize: 10, fontWeight: 700, color: colores.tickColor, opacity: 0.6 }}>
                {label}
              </span>
            </div>
          ))}
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 8, right: 16, left: 0, bottom: 0 }}>
            ...
            <XAxis
              ...
              axisLine={false}
              tickLine={false}
              tick={{ fill: colores.tickColor || '#918479', fontSize: 10, fontWeight: 700, fillOpacity: 0.6 }}
              dy={12}
            />
            ...
            <YAxis
              ...
              tickLine={false}
              width={48}
              tick={{ fill: colores.tickColor || '#918479', fontSize: 10, fontWeight: 700, textAnchor: 'end', fillOpacity: 0.6 }}
            ...
            <Area
              ...
              stroke={colores.ingresos} strokeWidth={2.5}
              fillOpacity={1} fill="url(#gradIngresos)"
              dot={false} activeDot={{ r: 5, fill: colores.ingresos, strokeWidth: 0 }}
              animationDuration={1200}
            />

            <Area
              type="monotone" dataKey="gastos" name="Gastos"
              stroke={colores.gastos} strokeWidth={2.5}
              fillOpacity={1} fill="url(#gradGastos)"
              dot={false} activeDot={{ r: 5, fill: colores.gastos, strokeWidth: 0 }}
              animationDuration={1200}
            />
          </AreaChart>
        </ResponsiveContainer>
