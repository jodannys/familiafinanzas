              ...
              {group.items.map(({ href, label, icon: Icon, color }) => {
                ...
                return (
                  <Link key={href} href={href} onClick={onClose}
                    ...
                    <Icon
                      size={22} // Un poco más grande para mejor visibilidad
                      strokeWidth={active ? 2.5 : 2}
                      className="transition-transform group-hover:scale-110"
                    />

                    {/* Tooltip (Desktop) */}
                    <span className="hidden lg:block absolute left-16 scale-0 group-hover:scale-100 transition-all z-50 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest text-white pointer-events-none shadow-2xl whitespace-nowrap"
                      style={{ 
                        background: '#111', 
