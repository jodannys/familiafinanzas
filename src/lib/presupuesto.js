...
export async function getPresupuestoMes(mesParam = null, añoParam = null) {
  ...
  try {
    ...
    const subMetas = (subData || []).find(s => s.categoria === 'metas')
    const subInversiones = (subData || []).find(s => s.categoria === 'inversiones')
    const pctMetas = subMetas ? subMetas.pct : 60
    const pctInversiones = subInversiones ? subInversiones.pct : 40
